...
<script type="text/javascript">
	...
	function deleteComment(commentNo, boardNo) {
		...
		if (yes) {
			location.href = 'deletecomment.action?commentno=' + commentNo
					+ "&boardno=" + boardNo;
		}
	}
	...
	function doDelete(boardNo, pageNo) {
		...
		if (yes) {
			location.href = 'delete.action?boardno=' + boardNo + '&pageno='
					+ pageNo;
		}
	}
				...
				<div id="inputmain">
					...
					<form action="write.action" method="post"
						...
						<table border="1" align="center">
							...
							<tr>
								<th>첨부파일</th>
								<td><input type="file" name="attach" style="height: 25px" />
								</td>
							</tr>
							<tr>
								<th>내용</th>
								<td><textarea name="content" cols="77" rows="15"></textarea></td>
							</tr>
						</table>
						<div class="buttons">
							...
							<c:forEach var="upload" items="${ uploads }" varStatus="status2">
								<c:if test="${ board.boardNo == upload.boardNo }" >
									<div border='1' align='center'>
										<img src='/team5/upload/${upload.savedFileName}' style="width:350px;height:350px" />
									</div>
								</c:if>
							</c:forEach>
						</div>
				...
				<div class="buttons">
					<c:choose>
						<c:when test="${loginuser.memberId == board.memberId}">
							<a href='javascript:doDelete(${board.boardNo})'><span class="glyphicon glyphicon-trash"></span> 삭제</a>&nbsp;&nbsp;
							<a id="boardeditlink${board.boardNo}" href='javascript:toggleBoardStatus(${board.boardNo},true)'>
									...
									<div id='commentedit${boardComment.commentNo}'
										...
										<form id="commenteditform${boardComment.commentNo}"
											...
											<input type="hidden" name="boardno"
												...
												name="commentno" value="${boardComment.commentNo}" />
